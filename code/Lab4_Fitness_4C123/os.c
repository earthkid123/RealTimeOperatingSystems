...
struct tcb{
  ...
  struct tcb *next;  // linked-list pointerint32_t *blocked; // nonzero if blocked on this semaphore
int32_t *blocked; // nonzero if blocked on this semaphore
 int32_t Sleep; // nonzero if this thread is sleeping//*FILL THIS IN****
	int32_t Priority;
};
...
//***IMPLEMENT THIS***
  ...
  while((SYSCTL_RCGCGPIO_R&0x08) ==0 ){};// allow time for clock to stabilize
  // 2) no need to unlock PD6
  edgeSemaphore = semaPt;
  GPIO_PORTD_AMSEL_R &= ~0x40; // 3) disable analog on PD6
  GPIO_PORTD_PCTL_R &= ~0x40;// 4) configure PD6 as GPIO
...
//***IMPLEMENT THIS***
  ...
  // step 3 disarm interrupt to prevent bouncing to create multiple signals
    ...
    GPIO_PORTD_ICR_R = 0x40;  // acknowledge flag4
    OS_Signal(edgeSemaphore);          // signal SW1 occurred
    GPIO_PORTD_IM_R &= ~0x40; // disarm interrupt on PF4
